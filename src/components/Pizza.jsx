...
class Pizza extends React.Component{
    ...
    state = {
     ...
     numberSelectedPizza:false,
      ...
      cbSelectedPizza=(boolean)=>{
        this.setState({numberSelectedPizza:boolean})
      }
      cbSelectedDough =(codeSelectedButton)=>{ 
       ...
       this.setState({price: Math.floor(this.props.price*1.45)})  
       if(codeSelectedSize===2)                   // 2-большой размер 
       this.setState({price: Math.floor(this.props.price*1.95)})
    ...
    <Dough cbSelectedPizza={this.cbSelectedPizza} key={elem.code} dough={elem.name}  code={elem.code} cbSelected={this.cbSelectedDough} selectedButton={this.state.newSelectedDough}></Dough>
     ))} 
    </ul>
    <ul>
      {this.state.sizes.map((elem,index)=>(
        <Size size={this.state.sizes} cbSelectedPizza={this.cbSelectedPizza} key={index} code={index} cbSelected={this.cbSelectedSize} selectedButton={this.state.newSelectedSize}/>
      )
        ...
        <div className='pizza__footer'>
          ...
          <svg style={{ marginLeft:-2}} width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
         ...
         <span > Добавить</span>
         {this.state.numberSelectedPizza? <span className='pizza__footer__button_selected'>{this.state.numberSelectedPizza}</span>:""}
         </div> 
